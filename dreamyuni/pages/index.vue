<template>
  <div class="min-h-screen bg-gradient-to-br from-gray-900 to-black text-white relative overflow-hidden">
    ...
    <main class="text-center py-20 px-4 relative z-10">
      ...
      <h1 class="text-5xl md:text-7xl font-bold mb-6 text-transparent bg-clip-text bg-gradient-to-r from-pastel-pink via-pastel-blue to-pastel-green">Elevate Your Learning</h1>
      <p class="text-lg mb-8 max-w-2xl mx-auto text-pastel-text">Unlock your potential with cutting-edge courses designed for the future.</p>
      <div class="flex justify-center space-x-4 mb-8">
        <button class="bg-gradient-to-r from-pastel-blue to-pastel-purple text-white px-6 py-3 rounded-full hover:shadow-neon transition duration-300">Explore Now</button>
        <button class="bg-transparent border-2 border-pastel-pink text-pastel-pink px-6 py-3 rounded-full hover:bg-pastel-pink hover:text-white transition duration-300">Join Us</button>
      </div>
      <div class="flex justify-center items-center">
        ...
        <span class="ml-2 text-pastel-green">+99 dreamers</span>
      </div>
    </main>

    <!-- Testimonials Section -->
    <section class="py-20 px-4 overflow-hidden relative z-10">
      <h2 class="text-3xl font-bold text-center mb-2 text-transparent bg-clip-text bg-gradient-to-r from-pastel-purple to-pastel-pink">Dreamer Testimonials</h2>
      <p class="text-center mb-12 text-pastel-text">Real stories from our community</p>
      <div class="relative">
          ...
          <div class="testimonial-container" :class="{ 'move-right': index % 2 === 0, 'move-left': index % 2 !== 0 }">
            <div v-for="testimonial in testimonials" :key="testimonial.id" class="testimonial-card">
              <div class="flex items-center">
                ...
                <div>
                  <h3 class="font-bold text-pastel-blue text-xs">{{ testimonial.name }}</h3>
                  <p class="text-xs text-pastel-text">{{ testimonial.handle }}</p>
                </div>
              </div>
              <p class="text-pastel-text text-xs mt-2 line-clamp-2">{{ testimonial.text }}</p>
            </div>
          </div>
        </div>
      </div>
